<?php
        ...
        */
	    ...
	    $connected = 1;
 /** 
          * $sql2       : Query to fetch updated / created call from group.
          * Date        : Jun-29-2018
          * Author      : Shubham 
          * PHP version : 5.6
          */
//Based on assingned_user
$sql2 = "SELECT  ca.id AS id, 
        ca.name AS name,
        ca.created_by AS created_by,
        ca.date_start AS date_start,
        ca.date_end AS date_end,
        ca.parent_type AS parent_type,
        ca.status AS status,
        ca.direction AS direction,
        ca.parent_id AS parent_id,
        ca.duration_hours AS duration_hours,
        ca.duration_minutes AS duration_minutes,
        ca.description AS description,
        ca.date_entered as date_entered,
        ca.date_modified as date_modified,
        ca.assigned_user_id as assigned_user_id,
        ca.reminder_time AS reminder_time,
        cac.date_modified_mobile_c AS date_modified_mobile_c,
        cac.mobile_id_c AS mobile_id_c,
        cac.mobile_offline_unique_id_c AS mobile_offline_unique_id,
        cac.call_notification_status_c AS call_notification_status,
        cac.calender_id_c AS calender_id_c,
        cac.record_source_c AS record_source_c,
...
FROM calls AS ca
LEFT JOIN calls_cstm AS cac ON cac.id_c = ca.id
LEFT JOIN users AS u ON ca.assigned_user_id = u.id
LEFT JOIN securitygroups_users AS seu ON ca.assigned_user_id=seu.user_id
INNER JOIN securitygroups AS sg ON sg.id=seu.securitygroup_id
WHERE seu.securitygroup_id in (SELECT securitygroup_id FROM securitygroups_users WHERE user_id='".$assigned_user_id."')  AND cac.deleted_from_mobileapp_c = '0' AND ca.deleted = '0' AND ca.date_modified >=  '".$date_modified_sugar_format_sync."' order by ca.date_modified DESC";
