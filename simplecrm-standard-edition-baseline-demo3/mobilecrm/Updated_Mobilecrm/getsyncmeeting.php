<?php
            ...
            */
    	    ...
    	    $connected = 1;
    /** 
      * $sql2       : Query to fetch updated / created meeting from group.
      * Date        : Jun-28-2018
      * Author      : Shubham 
      * PHP version : 5.6
      */
//Based on assingned_user
$sql2 = "SELECT   me.id AS id,
         me.name AS name, me.parent_type AS parent_type,me.created_by AS created_by,
         me.parent_id AS parent_id, me.status AS status,me.date_start AS date_start,
         me.date_end AS date_end,me.description AS description, me.location AS location,
         me.assigned_user_id AS assigned_user_id,
         me.date_entered as date_entered,me.date_modified as date_modified,
         u.user_name AS user_name, 
         me.reminder_time AS reminder_time, 
         mec.mobile_offline_unique_id_c AS mobile_offline_unique_id,
         mec.meeting_notification_status_c AS meeting_notification_status,
         mec.calender_id_c AS calender_id_c, mec.record_source_c AS record_source_c,
         mec.date_modified_mobile_c AS date_modified_mobile_c,
         mec.mobile_id_c AS mobile_id_c,
         CONCAT( u.first_name , ' ', u.last_name ) AS assigned_user_full_name,
         me.duration_hours AS duration_hours,me.duration_minutes AS duration_minutes,
         mec.duration_c AS duration_c,
...
LEFT JOIN securitygroups_users AS seu ON me.assigned_user_id=seu.user_id
INNER JOIN securitygroups AS sg ON sg.id=seu.securitygroup_id
WHERE seu.securitygroup_id in (SELECT securitygroup_id FROM securitygroups_users WHERE user_id='".$assigned_user_id."') AND mec.deleted_from_mobileapp_c = '0'  AND me.deleted = '0' AND me.date_modified >=  '".$date_modified_sugar_format_sync."' order by me.date_modified DESC";
